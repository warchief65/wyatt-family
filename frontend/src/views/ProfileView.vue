<template>
  <div class="profile-page">
    <div class="page-header">
      <h1 class="display-font page-title">My Profile</h1>
      <hr class="gold-rule" />
    </div>
    ...
    <div class="profile-content">
      <div class="profile-card card">
        ...
        <div class="profile-email text-muted">{{ auth.user?.email }}</div>
        <span class="badge" :class="`badge-${auth.user?.status}`">{{ auth.user?.status }}</span>
      </div>

      <div class="submissions-section">
        <h2 class="section-label">My Submissions</h2>
        <hr class="gold-rule" />
        <div v-if="!submissions.length" class="empty text-muted">You haven't submitted any content yet.</div>
        <div v-else class="submissions-list">
          <div v-for="s in submissions" :key="s.id" class="sub-row card">
            <div class="sub-info">
              <div class="sub-title">{{ s.title }}</div>
              <div class="sub-meta text-muted">{{ s.contentType }} · {{ formatDate(s.submittedAt) }}</div>
              <div v-if="s.rejectionReason" class="rejection-reason text-muted">{{ s.rejectionReason }}</div>
            </div>
            <span :class="`badge badge-${s.status}`">{{ s.status }}</span>
          </div>
        </div>
        <RouterLink to="/submit" class="btn btn-secondary mt-2">Submit More Content</RouterLink>
      </div>
    </div>
  </div>
</template>
